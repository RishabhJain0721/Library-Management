...
const AmberButton = (props) => {
  const navigate = useNavigate();

  return (
    <button
      disabled={!!props.loading}
      className="bg-amber-800 text-white transition-all rounded-full text-xl py-2 px-4 mt-4 hover:bg-amber-900 disabled:opacity-80 inline-flex"
      onClick={() => navigate(props.navigation)}
    >
      {props.loading ? (
...
export { WhiteButton, YellowButton, AmberButton };
